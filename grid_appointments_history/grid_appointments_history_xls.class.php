...
function actionBar_getStateHide($buttonName)
{
   ...
   function inicializa_vars()
   {
      ...
      { 
          ...
          if (!is_file($this->Ini->root . $this->Ini->path_link . "grid_appointments_history/grid_appointments_history_res_xls.class.php"))
          {
              $this->Tem_xls_res  = false;
          }
          if ($this->Tem_xls_res)
          { 
              require_once($this->Ini->path_aplicacao . "grid_appointments_history_res_xls.class.php");
              $this->Res_xls = new grid_appointments_history_res_xls();
      ...
      if (!$_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_history']['embutida'] && !$this->Ini->sc_export_ajax) {
          ...
          $this->pb->setReturnOption($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_history']['xls_return']);
          if ($this->Tem_xls_res) {
   ...
   function grava_arquivo()
   {
     ...
     $nm_houve_quebra = "N";
      ...
      { 
          if ($this->Tem_xls_res)
          { 
              $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_history']['xls_res_grid'] = true;
              if (!$this->Ini->sc_export_ajax) {
              ...
              if ($this->Use_phpspreadsheet) {
                  $Xls_res = \PhpOffice\PhpSpreadsheet\IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_history']['xls_res_sheet']);
              }
              else {
                  $Xls_res = PHPExcel_IOFactory::load($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_history']['xls_res_sheet']);
              }
              foreach($Xls_res->getAllSheets() as $sheet)
              {
                  $this->Xls_dados->addExternalSheet($sheet);
              }
              unset($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_history']['xls_res_grid']);
              unlink($_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_history']['xls_res_sheet']);
          } 
          if (!$this->Ini->sc_export_ajax) {
   ...
   function NM_export_appointments_history_date()
   {
      ...
      if (!empty($this->appointments_history_date))
      {
             if (substr($this->appointments_history_date, 10, 1) == "-") 
             { 
                 $this->appointments_history_date = substr($this->appointments_history_date, 0, 10) . " " . substr($this->appointments_history_date, 11);
             } 
             if (substr($this->appointments_history_date, 13, 1) == ".") 
             { 
                $this->appointments_history_date = substr($this->appointments_history_date, 0, 13) . ":" . substr($this->appointments_history_date, 14, 2) . ":" . substr($this->appointments_history_date, 17);
             } 
             $conteudo_x =  $this->appointments_history_date;
             nm_conv_limpa_dado($conteudo_x, "YYYY-MM-DD HH:II:SS");
             if (is_numeric($conteudo_x) && strlen($conteudo_x) > 0) 
             { 
                 $this->nm_data->SetaData($this->appointments_history_date, "YYYY-MM-DD HH:II:SS  ");
                 $this->appointments_history_date = $this->nm_data->FormataSaida($this->nm_data->FormatRegion("DH", "ddmmaaaa;hhiiss"));
             } 
      }
