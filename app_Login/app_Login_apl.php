...
else if($this->rs[0][1] == 'Y')
{
	...
	switch ($usr_group){
		case 2:
			...
			staff_email			
			FROM staff 
			WHERE sec_users_login = '".$usr_login."'";	
      ...
      { 
          ...
          $this->rs2_erro = $this->Db->ErrorMsg();
			...
			$usr_name		= $this->rs2[0][1];
			$usr_email		= $this->rs2[0][2];
			$glo_staff_id   = $this->rs2[0][0];
...
;
      ...
      { 
          ...
          $this->rs2_erro = $this->Db->ErrorMsg();
			...
			$usr_name		= $this->rs2[0][1];
			$usr_email		= $this->rs2[0][2];
			$glo_customer_id   = $this->rs2[0][0];
			 if (isset($glo_customer_id)) {$this->sc_temp_glo_customer_id = $glo_customer_id;}
;
		  break;
...
<?php
  ...
  if(isset($_SERVER['HTTP_HOST']) && substr($_SERVER['HTTP_HOST'], 0, 14) == "www.scriptcase") {
	...
	$this->Ini->sc_set_schema('Sc9_SweetBlue');;
