...
class grid_appointments_adm_total
{
   ...
   function Calc_resumo_sc_free_group_by($destino_resumo, $res_export=false)
   {
      ...
      $cmp_sql_def   = array('staff_id' => "staff_id",'customers_id' => "customers_id",'appointment_start_date' => "appointment_start_date",'appointment_start_time' => "appointment_start_time");
   ...
   function Res_Totaliza_sc_free_group_by($ind_qb, $cmp_tot, $arr_tots, $group, $join, $cmps_quebras, $cmps_quebras1, $cmps_quebras2, $cmps_quebrasS, $Cmp_dim, $cmps_quebra_atual, $cmp_sql_def, $res_export)
   {
      ...
      { 
         $cmd_simp = "select count(*), sum(total_price)#@#cmps_quebras#@# from " . $this->Ini->nm_tabela . " " . $where_ok;
         $comando  = "select count(*), sum(SC_metric1)#@#cmps_quebras#@# from (";
         $comando .= "select total_price as SC_metric1, " . $cmps_quebras1 . " from " . $this->Ini->nm_tabela . " " .  $where_ok . ") SC_sel1 INNER JOIN (";
      ...
      $format_dimensions['customers_id']['reg'] = "S";
      $format_dimensions['customers_id']['msk'] = "";
      $format_dimensions['appointment_start_date']['reg'] = "S";
      $format_dimensions['appointment_start_date']['msk'] = "";
      $format_dimensions['appointment_start_time']['reg'] = "S";
      $format_dimensions['appointment_start_time']['msk'] = "";
      while (!$rt->EOF)
      {
          $sql_where = "";
          foreach ($Cmp_dim as $Cada_dim => $Ind_sql)
          {
              ...
              if ($Cada_dim == "customers_id") {
                  $this->Lookup->lookup_sc_free_group_by_customers_id($$Cada_dim,  $customers_id);
              }
              if (null === $$Cada_dim)
              {
                  $$Cada_dim = '';
   ...
   {
      global $nada, $nm_lang , $staff_id, $customers_id, $current_status_id;
      ...
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(total_price) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['where_pesq']; 
      } 
      elseif (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_mssql))
      { 
          $nm_comando = "select count(*), sum(total_price) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['where_pesq']; 
      ...
      $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['tot_geral'][0] = "" . $this->Ini->Nm_lang['lang_msgs_totl'] . ""; 
   ...
   function quebra_staff_id_sc_free_group_by($staff_id, $Where_qb, $Cmp_Name) 
   {
      ...
      global $$Var_name_gb, $staff_id, $customers_id, $current_status_id;
      $tot_staff_id = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(total_price) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['where_pesq']; 
   ...
   function quebra_customers_id_sc_free_group_by($customers_id, $Where_qb, $Cmp_Name) 
   {
      ...
      $tot_customers_id[0] = NM_encode_input(sc_strip_script($customers_id)) ; 
      $tot_customers_id[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_customers_id[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_customers_id;
   } 
   ...
   function quebra_appointment_start_time_sc_free_group_by($appointment_start_time, $Where_qb, $Cmp_Name) 
   {
      $Var_name_gb = "SC_tot_" . $Cmp_Name;
      global $$Var_name_gb, $staff_id, $customers_id, $current_status_id;
      $tot_appointment_start_time = array() ;  
      if (in_array(strtolower($this->Ini->nm_tpbanco), $this->Ini->nm_bases_access))
      { 
          $nm_comando = "select count(*), sum(total_price) from " . $this->Ini->nm_tabela . " " . $_SESSION['sc_session'][$this->Ini->sc_page]['grid_appointments_adm']['where_pesq']; 
      } 
      ...
      $tot_appointment_start_time[0] = NM_encode_input(sc_strip_script($appointment_start_time)) ; 
      $tot_appointment_start_time[1] = $rt->fields[0] ; 
      $rt->fields[1] = str_replace(",", ".", $rt->fields[1]);
      $tot_appointment_start_time[2] = (string)$rt->fields[1]; 
      $rt->Close(); 
      $$Var_name_gb = $tot_appointment_start_time;
   } 

   //-----  current_status_id
   function quebra_current_status_id_status($current_status_id, $arg_sum_current_status_id) 
   {
      global $tot_current_status_id, $staff_id, $customers_id, $current_status_id;
