<div id="form_appointments_form2" style='<?php echo ($this->tabCssClass["form_appointments_form2"]['class'] == 'scTabInactive' ? 'display: none; width: 1px; height: 0px; overflow: scroll' : ''); ?>'>
...
<TABLE align="center" id="hidden_bloco_4" class="scFormTable<?php echo $this->classes_100perc_fields['table'] ?>" width="100%" style="height: 100%;"><?php if ($sc_hidden_no > 0) { echo "<tr>"; }; 
   ...
   <?php
    if (!isset($this->nm_new_label['hist']))
    {
        $this->nm_new_label['hist'] = "hist";
    }
?>
<?php
   ...
   $hist = $this->hist;
   $sStyleHidden_hist = '';
   if (isset($this->nmgp_cmp_hidden['hist']) && $this->nmgp_cmp_hidden['hist'] == 'off')
   {
       unset($this->nmgp_cmp_hidden['hist']);
       $sStyleHidden_hist = 'display: none;';
   }
   $bTestReadOnly = true;
   $sStyleReadLab_hist = 'display: none;';
   $sStyleReadInp_hist = '';
   if (/*$this->nmgp_opcao != "novo" && */isset($this->nmgp_cmp_readonly['hist']) && $this->nmgp_cmp_readonly['hist'] == 'on')
   {
       $bTestReadOnly = false;
       unset($this->nmgp_cmp_readonly['hist']);
       $sStyleReadLab_hist = '';
       $sStyleReadInp_hist = 'display: none;';
   }
?>
<?php if (isset($this->nmgp_cmp_hidden['hist']) && $this->nmgp_cmp_hidden['hist'] == 'off') { $sc_hidden_yes++;  ?>
<input type="hidden" name="hist" value="<?php echo $this->form_encode_input($hist) . "\">"; ?>
<?php } else { $sc_hidden_no++; ?>

    <TD class="scFormDataOdd css_hist_line" id="hidden_field_data_hist" style="<?php echo $sStyleHidden_hist; ?>vertical-align: top;"> <table style="border-width: 0px; border-collapse: collapse; width: 100%"><tr><td width="100%" class="scFormDataFontOdd css_hist_line" style="vertical-align: top;padding: 0px">
<?php
 if (isset($_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_hist'] ]) && '' != $_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_hist'] ]) {
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['grid_appointments_history_script_case_init'] = $_SESSION['scriptcase']['dashboard_scinit'][ $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['dashboard_info']['dashboard_app'] ][ $this->Ini->sc_lig_target['C_@scinf_hist'] ];
 }
 else {
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['grid_appointments_history_script_case_init'] = $this->Ini->sc_page;
 }
 ...
 if (isset($this->Ini->sc_lig_md5["grid_appointments_history"]) && $this->Ini->sc_lig_md5["grid_appointments_history"] == "S") {
     $Parms_Lig  = "var_appointment_id*scin" . $this->nmgp_dados_form['appointments_id'] . "*scout";
     $Md5_Lig    = "@SC_par@" . $this->form_encode_input($this->Ini->sc_page) . "@SC_par@form_appointments@SC_par@" . md5($Parms_Lig);
     $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['Lig_Md5'][md5($Parms_Lig)] = $Parms_Lig;
 } else {
     $Md5_Lig  = "var_appointment_id*scin" . $this->nmgp_dados_form['appointments_id'] . "*scout";
 }
 $parms_lig_cons = "&nmgp_parms=" . $Md5_Lig;
 $sDetailSrc = ('novo' == $this->nmgp_opcao) ? 'form_appointments_empty.htm' : $this->Ini->link_grid_appointments_history_cons . '?script_case_init=' . $this->form_encode_input($this->Ini->sc_page) . '&script_case_detail=Y' . $parms_lig_cons;
if (isset($this->Ini->sc_lig_target['C_@scinf_hist']) && 'nmsc_iframe_liga_grid_appointments_history' != $this->Ini->sc_lig_target['C_@scinf_hist'])
{
    if ('novo' != $this->nmgp_opcao)
    {
        $sDetailSrc .= '&under_dashboard=1&dashboard_app=' . $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['dashboard_info']['dashboard_app'] . '&own_widget=' . $this->Ini->sc_lig_target['C_@scinf_hist'] . '&parent_widget=' . $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['dashboard_info']['own_widget'];
        $sDetailSrc  = $this->addUrlParam($sDetailSrc, 'script_case_init', $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['grid_appointments_history_script_case_init']);
    }
?>
<script type="text/javascript">
$(function() {
    scOpenMasterDetail("<?php echo $this->Ini->sc_lig_target['C_@scinf_hist'] ?>", "<?php echo $sDetailSrc; ?>");
});
</script>
<?php
}
...
{
?>
<iframe border="0" id="nmsc_iframe_liga_grid_appointments_history"  marginWidth="0" marginHeight="0" frameborder="0" valign="top" height="100" width="600" name="nmsc_iframe_liga_grid_appointments_history"  scrolling="auto" src="<?php echo $sDetailSrc; ?>"></iframe>
<?php
}
?>
</td></tr><tr><td style="vertical-align: top; padding: 0"><table class="scFormFieldErrorTable" style="display: none" id="id_error_display_hist_frame"><tr><td class="scFormFieldErrorMessage"><span id="id_error_display_hist_text"></span></td></tr></table></td></tr></table> </TD>
   <?php }?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['btn_disabled']['new']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['btn_disabled']['new']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['btn_label']['new']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['btn_label']['new']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['btn_label']['new'];
        }
?>
...
<?php
        ...
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['btn_disabled']['insert']) && 'on' == $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['btn_disabled']['insert']) {
            $buttonMacroDisabled .= ' disabled';
        }
        if (isset($_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['btn_label']['insert']) && '' != $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['btn_label']['insert']) {
            $buttonMacroLabel = $_SESSION['sc_session'][$this->Ini->sc_page]['form_appointments']['btn_label']['insert'];
        }
?>
...
<script> 
<?php
 $NM_pag_atual = "form_appointments_form0";
 if (isset($this->nmgp_ancora) && $this->nmgp_ancora != "")
 {
     $NM_pag_atual = "form_appointments_form" . $this->nmgp_ancora;
 }
?>
<?php
